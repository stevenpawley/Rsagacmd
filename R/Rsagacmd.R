...
#' version has been tested using SAGA-GIS 2.3.2, 5.0.0, 6.0.0-6.3.0 on Windows
#' (x64), OS X and Linux. Rsagacmd provides a fully functional approach of
...
#' structure facilitates alibrn easier scripting experience by organizing the
...
#' library(magrittr)
#'
#' # Check if SAGA-GIS is installed in a recognizable location
#' if(!is.null(sagaSearch())) {
#' 
#'    # initialize a saga object with a subset of available libraries
#'    saga = sagaGIS(opt_lib = c('grid_calculus', 'ta_morphometry'))
#'    
#'    # Generate random terrain and automatically use of tempfile to store
#'    # tool outputs
#'    dem = saga$grid_calculus$Random_Terrain(ITERATIONS = 1)
#'  
