...
#' emphasizes an encapuled object-oriented method of scripting with SAGA-GIS by
#' dynamically generating R functions for every SAGA-GIS tool based on the
#' user's current SAGA installation. These functions are generated by the
#' \code{\link{sagaGIS}} function and are contained within an R6 object as a
#' named list of libraries and tools. This facilitates an easier scripting
#' experience by organizing the large number of SAGA-GIS tools (>500) by their
#' respective library. Each function's syntax is therefore similar to what is
#' encounted when using the SAGA-GIS command line tool directly. Furthermore,
#' because the arguments (called identifiers) for many SAGA-GIS tools are not
#' consistently named, the user can also take advantage of code autocompletion
#' tools (e.g. in \href{http://rstudio.com}{Rstudio}), allowing for each tools'
#' inputs, outputs and options to be more quickly recognized.
#' 
