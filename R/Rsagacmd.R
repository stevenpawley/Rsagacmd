...
#' alternative interface to SAGA-GIS. In contrast to RSAGA, Rsagacmd provides an
#' object-oriented method of scripting with the open-source System for Automated
#' Geoscientific Analyses Geographical Information System software (SAGA-GIS).
#' Rsagacmd dynamically generates R functions for every SAGA-GIS tool based on
#' the user's current SAGA-GIS installation. These functions are generated by
#' the \code{\link{sagaGIS}} function and are included within an R6 object as a
#' named list of libraries and tools. This structure facilitates an easier
#' scripting experience by organizing the large number of SAGA-GIS tools (>500)
#' by their respective library. Interactive scripting can also fully take
#' advantage of code autocompletion tools (e.g. in
