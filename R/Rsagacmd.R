...
#' System for Automated Geoscientific Analyses Geographical Information System
#' software (SAGA-GIS). Rsagacmd dynamically generates R functions for every
...
#' contained within an R6 object and are accessed as a named list of libraries
#' and tools. This structure facilitates an easier scripting experience by
#' organizing the large number of SAGA-GIS tools (>500) by their respective
#' library. Interactive scripting can therefore fully take advantage of code
#' autocompletion tools (e.g. in \href{http://rstudio.com}{Rstudio}), allowing
#' for each tool's syntax to be quickly recognized. Furthermore, the most common
#' types of spatial data (rasters using the \pkg{raster} package, and vector
#' data using the \pkg{sp} or simple features \pkg{sf} packages) along with
#' non-spatial data are seamlessly passed from R to the SAGA-GIS command line
#' tool for geoprocessing operations, and the results are automatically loaded
#' as the appropriate R object. Outputs from individual SAGA-GIS tools can also
#' be chained using pipes from the \pkg{magrittr} and \pkg{dplyr} packages to
#' chain complex geoprocessing operations together in a single statement.
#' 
