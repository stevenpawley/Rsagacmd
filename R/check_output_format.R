...
check_output_format <- function(x, raster_format, vector_format) {
  if (x$feature == "Grid") {
    ...
    if (ext != raster_format) {
      msg <- paste("`raster_format` is set to", quote_file_ext(raster_format), 
                   "but you specified an output with a", quote_file_ext(ext), 
                   "file extension.",
                   "This will prevent Rsagacmd from loading the output into your R environment")
    ...
    if (ext != vector_format) {
      msg <- paste("`vector_format` is set to", quote_file_ext(vector_format),
                   "but you specified an output with a", quote_file_ext(ext),
                   "file extension.",
                   "This will prevent Rsagacmd from loading the output into your R environment")
      rlang::abort(msg)
    ...
    if (any(ext != vector_format)) {
      msg <- paste("`raster_format` is set to", quote_file_ext(vector_format), 
                   "but you specified an output with a", 
                   quote_file_ext(incorrect_ext), "file extension.", 
                   "This will prevent Rsagacmd from loading the output into your R environment")
      rlang::abort(msg)
