...
-   Raster data handling is provided by the R **raster** package. Raster-based
outputs from SAGA-GIS tools are loaded as `RasterLayer` objects. For more
details, see the 'Handling of raster data'.
-   Vector features that result from SAGA-GIS geoprocessing operations are
output in ESRI Shapefile format and are loaded into the R environment as simple
features (`sf`) objects.
